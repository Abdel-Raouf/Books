...
#define SUCCESS			0
 ...
 int  lin_net2_module_init(void);

 void lin_net2_module_exit(void);

 void  lin_net2_init(struct net_device *dev);

 int  lin_net2_open(struct net_device *dev);

 int  lin_net2_stop(struct net_device *dev);

 int  lin_net2_set_config(struct net_device *dev,
				struct ifmap *map);

 int  lin_net2_hard_start_xmit(struct sk_buff *skb,
				struct net_device *dev);

 int  lin_net2_do_ioctl(struct net_device *dev,
				struct ifreq *ifr, int cmd);

 struct net_device_stats* lin_net2_get_stats(
				struct net_device *dev);

 int  lin_net2_change_mtu(struct net_device *dev,
				int new_mtu);

 int  lin_net2_rebuild_header(struct sk_buff *skb);

 int  lin_net2_hard_header(struct sk_buff *skb,
				struct net_device *dev,
				unsigned short type,
				void *daddr,
				void *saddr,
 ...
 void  lin_net2_tx_timeout(struct net_device *dev);

 int   lin_net2_hw_tx(char *data, int len,
				struct net_device *dev);

 void  lin_net2_interrupt(int irq, void *dev_id,
				struct pt_regs *regs);
...
{
	int error;
	lin_net2_dev=alloc_netdev(sizeof(lin_net2_priv_t),"lin2",lin_net2_init);
...
/*	dev->priv = kmalloc( sizeof(lin_net2_priv_t), GFP_KERNEL );
	...
	memset( priv, 0, sizeof(lin_net2_priv_t) );

	/* step 5 */
	spin_lock_init( &lin_net2_lock );
}

/* We must know that this function is called when ifconfig net2 up
...
{
	/* step 1 */
	memcpy(dev->dev_addr, "\0lin1", ETH_ALEN);
	/* step 2 */
...
/* We must know that this function is called when ifconfig net2 down
				...
				struct ifmap *map)
{
	...
	if( map->base_addr != dev->base_addr ) {
		printk("lin2: Cant change I/O Address base\n");
		return -EOPNOTSUPP;
	}
...
*/
 write_to_hardware(lin_net2_d, dstaddr, data, len);
	/* step 5 */
 ...
 int  lin_net2_rebuild_header(struct sk_buff *skb)
{
	...
	eth->h_dest[ETH_ALEN-1] = '1'; /*lin_net2 -> lin_net2*/
	return 0;
}
