...
#define SUCCESS			0
 ...
 int  lin_net1_module_init(void);

 void lin_net1_module_exit(void);

 void  lin_net1_init(struct net_device *dev);

 int  lin_net1_open(struct net_device *dev);

 int  lin_net1_stop(struct net_device *dev);

 int  lin_net1_set_config(struct net_device *dev,
				struct ifmap *map);

 int  lin_net1_hard_start_xmit(struct sk_buff *skb,
				struct net_device *dev);

 int  lin_net1_do_ioctl(struct net_device *dev,
				struct ifreq *ifr, int cmd);

 struct net_device_stats* lin_net1_get_stats(
				struct net_device *dev);

 int  lin_net1_change_mtu(struct net_device *dev,
				int new_mtu);

 int  lin_net1_rebuild_header(struct sk_buff *skb);

 int  lin_net1_hard_header(struct sk_buff *skb,
				struct net_device *dev,
				unsigned short type,
				void *daddr,
				void *saddr,
 ...
 void  lin_net1_tx_timeout(struct net_device *dev);

 int   lin_net1_hw_tx(char *data, int len,
				struct net_device *dev);

 void  lin_net1_interrupt(int irq, void *dev_id,
				struct pt_regs *regs);
...
{
	int error;
	lin_net1_dev=alloc_netdev(sizeof(lin_net1_priv_t),"lin1",lin_net1_init);
...
/*	dev->priv = kmalloc( sizeof(lin_net1_priv_t), GFP_KERNEL );
	...
	memset( priv, 0, sizeof(lin_net1_priv_t) );
...
/* We must know that this function is called when ifconfig net1 up
   ...
   1. assign hardware address "\0lin00"
   2. Only in this case register pseudo hardware device
   3. start device queue
*/
 int  lin_net1_open(struct net_device *dev)
{
...
*/
 write_to_hardware(lin_net1_d, dstaddr, data, len);
