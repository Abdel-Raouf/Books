...
// defined in lin_net_device.h file 
//typedef void (pn_interrupt_fn)(int irq, void *dev_id,struct pt_regs *regs);
...
int register_pseudo_net_device(pn_interrupt_fn *fn, 
			struct net_device *dev)
{
	int i;
...
EXPORT_SYMBOL(register_pseudo_net_device);

/* unregisters a registered device */
int unregister_pseudo_net_device(int nd)
{
	if( nd > MAX_DEVICES )
		return -EINVAL;
...
EXPORT_SYMBOL(unregister_pseudo_net_device);

/* writes data to another device with ethernet address dev_addr */
int write_to_hardware(int nd, char *dev_addr, char *data, int len)
{
	int i;
...
EXPORT_SYMBOL(write_to_hardware);

/* reads data from the device */
int read_from_hardware(int nd, char **data, int *len)
{
...
EXPORT_SYMBOL(read_from_hardware);
