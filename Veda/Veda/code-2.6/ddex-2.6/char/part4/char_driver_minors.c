...
struct semaphore vsem0;
struct semaphore vsem1;
struct semaphore vsem2;
static int init_char_device(void)
{
	unsigned int i,ret;
	...
	for(i=0;i<3;i++)
        class_simple_device_add(veda_class,MKDEV(char_device_id,i),NULL,"veda_cdrv%d",i);
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,14)
	...
	for(i=0;i<3;i++)
        class_device_create(veda_class,MKDEV(char_device_id,i),NULL,"veda_cdrv%d",i);
#endif
...
#else
	for(i=0;i<3;i++)
	class_device_destroy(veda_class,MKDEV(char_device_id,i));
			    ...
			    loff_t *ppos)
{
	...
	{
		case 0:
			if(atomic_read(&vsem0.count))
			{
				if( lbuf > MAX_LENGTH )
				{
					bytes_to_do = MAX_LENGTH;
				}
				else
					bytes_to_do = lbuf;
				...
				*ppos = 0;
				...
				down(&vsem0);
				ret = nbytes = bytes_to_do - copy_to_user( buf, /* to */
			    ...
			    loff_t *ppos)
{
	...
	case 0:
		if(atomic_read(&vsem0.count))
		{
			...
			*ppos = 0;
			down(&vsem0);
			ret = nbytes = bytes_to_do - copy_from_user( char_device_buf_minor0, /* to */
														buf, /* from */
														bytes_to_do ); /* how many bytes */
			up(&vsem0);
	...
	case 1:
        if(atomic_read(&vsem1.count))
        {
			...
			*ppos = 0;
			down(&vsem1);
			ret = nbytes = bytes_to_do - copy_from_user( char_device_buf_minor1, /* to */
														buf, /* from */
														bytes_to_do ); /* how many bytes */
			up(&vsem1);
			/* wake up readers that are waiting for this data */
