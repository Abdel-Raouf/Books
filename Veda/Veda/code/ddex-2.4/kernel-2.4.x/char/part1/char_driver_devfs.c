/*
 * Character Device Driver with devfs support
 * =============================================
 *
 * This is same as the other char_driver.c but it automatically
 * creates device nodes in /dev directory
 * Team: Veda
 * Veresion: 1.0
 * */
...
static devfs_handle_t veda_devfs_dir, veda_devfs_device;
...
static int init_char_device(void)
{
	...
	/* devfs code */
	veda_devfs_dir = devfs_mk_dir(NULL, "veda", NULL);
	if( !veda_devfs_dir ) {
		printk("veda_devfs_dir: failed\n");
		return -EBUSY;
	}
	/* notice that we do not know the major number */
	veda_devfs_device = devfs_register( veda_devfs_dir, 
			CHAR_DEVICE_NAME, 
			DEVFS_FL_AUTO_DEVNUM,
			0, 0, S_IFCHR|S_IRUGO|S_IWUGO,
			(void *)&char_device_file_ops,
			NULL);
	if( !veda_devfs_device ) {
		printk("veda_devfs_device: failed\n");
		devfs_unregister(veda_devfs_dir);
		return -EBUSY;
	}
...
{
	devfs_unregister(veda_devfs_device);
	devfs_unregister(veda_devfs_dir);
	/*int id;
