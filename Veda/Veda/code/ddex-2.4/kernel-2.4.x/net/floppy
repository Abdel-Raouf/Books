...
/*
The network interface will be called lin1
Hardware Address of this will be: "\0lin00"
*/
...
#define lin_NET1_TIMEOUT	5 /*jiffies*/
#define SUCCESS			0

static __init int  lin_net1_module_init(void);

static __exit void lin_net1_module_exit(void);

static __exit int  lin_net1_init(struct net_device *dev);

static __exit int  lin_net1_open(struct net_device *dev);

static __exit int  lin_net1_stop(struct net_device *dev);

static __exit int  lin_net1_set_config(struct net_device *dev,
				struct ifmap *map);

static __exit int  lin_net1_hard_start_xmit(struct sk_buff *skb,
				struct net_device *dev);

static __exit int  lin_net1_do_ioctl(struct net_device *dev,
				struct ifreq *ifr, int cmd);

static __exit struct net_device_stats* lin_net1_get_stats(
				struct net_device *dev);

static __exit int  lin_net1_change_mtu(struct net_device *dev,
				int new_mtu);

static __exit int  lin_net1_rebuild_header(struct sk_buff *skb);

static __exit int  lin_net1_hard_header(struct sk_buff *skb,
				struct net_device *dev,
				unsigned short type,
				void *daddr,
				void *saddr,
...
static __exit void  lin_net1_tx_timeout(struct net_device *dev);

static __exit int   lin_net1_hw_tx(char *data, int len,
				struct net_device *dev);

static __exit void  lin_net1_interrupt(int irq, void *dev_id,
				struct pt_regs *regs);

module_init(lin_net1_module_init);
module_exit(lin_net1_module_exit);

struct net_device lin_net1_dev;

/* Declare our private data structure */
typedef struct __lin_net1_priv {
...
#define PRIV(X)  ((lin_net1_priv_t *)(X)->priv)
#define STATS(X) (PRIV((X))->stats)

/* Spin lock used for protecting critical code */
static spinlock_t lin_net1_lock;
static unsigned long lin_net1_flags;
#define lin_LOCK  	spin_lock_irqsave(&lin_net1_lock, lin_net1_flags)
#define lin_UNLOCK	spin_unlock_irqrestore(&lin_net1_lock, lin_net1_flags)

/* Pseudo Netdevice Descriptor */
static int lin_net1_d = -1;

/* Responsonility of lin_net1_module_init or init_module
   1. Copy the correct name on to lin_net1_dev.name.
   2. Register device using register_netdev which calls lin_net1_init
*/
static __init int  lin_net1_module_init(void)
{
	int error;
	/* step 1 */
	lin_net1_dev.init = lin_net1_init;
	strcpy(lin_net1_dev.name, "lin1");

	/* step 2 */
	if( (error = register_netdev(&lin_net1_dev)) ) {
		printk("Network device registration failure %d\n", error);
		return -ENODEV;
	}
	printk("lin_net1 loaded\n");
	return SUCCESS;
}
...
{
	/* step 1 */
	kfree(lin_net1_dev.priv);
	/* step 2 */
	unregister_netdev( &lin_net1_dev );
	printk("lin_net1 unloaded\n");
}
...
/* Responsibilities of device initialization function
   ...
   4. Allocate space for lin_net1_priv_t datatype
   5. Initialize our spin lock lin_net1_lock
*/
static __exit int  lin_net1_init(struct net_device *dev)
{
	/* step 1 */
	...
	/* step 2 */
	dev->open		= lin_net1_open;
	dev->stop		= lin_net1_stop;
	dev->set_config		= lin_net1_set_config;
	dev->hard_start_xmit	= lin_net1_hard_start_xmit;
	dev->do_ioctl		= lin_net1_do_ioctl;
	dev->get_stats		= lin_net1_get_stats;
	dev->rebuild_header	= lin_net1_rebuild_header;
	dev->hard_header	= lin_net1_hard_header;
	dev->tx_timeout		= lin_net1_tx_timeout;
	dev->watchdog_timeo	= lin_NET1_TIMEOUT;
	dev->flags		|= IFF_NOARP;
	...
	/* step 4 */
	dev->priv = kmalloc( sizeof(lin_net1_priv_t), GFP_KERNEL );
	if( dev->priv == NULL )
		return -ENOMEM;
	memset( dev->priv, 0, sizeof(lin_net1_priv_t) );

	/* step 5 */
	spin_lock_init( &lin_net1_lock );
	return 0;
}
...
/* We must know that this function is called when ifconfig net1 up
   ...
   2. assign hardware address "\0lin00"
...
static __exit int  lin_net1_open(struct net_device *dev)
{
	...
	MOD_INC_USE_COUNT;
	/* step 2 */
	memcpy(dev->dev_addr, "\0lin00", ETH_ALEN);
	/* step 3 */
	lin_net1_d = register_pseudo_net_device(lin_net1_interrupt, dev );
	if( lin_net1_d < 0 )
		return -ENODEV;
	/* step 4 */
...
{
	/* step 1 */
	unregister_pseudo_net_device(lin_net1_d);
	/* step 2 */
				...
				struct net_device *dev)
{
	...
	lin_net1_priv_t *priv = (lin_net1_priv_t *)dev->priv;
	...
	/* step 4 */
	retval = lin_net1_hw_tx(data, len, dev);
...
/* This is where the actual transmission of packet takes place.
   The data, len and dev is sent to us by lin_net1_hard_start_xmit
				...
				struct net_device *dev)
{
	...
	/* step 4 */
	if( (retval = write_to_hardware(lin_net1_d, dstaddr, data, len)) < 0 ) {
		STATS(dev).tx_errors++;
		return retval;
	}
...
/* Rebuild header is called just before packet transmission
   ...
   We make a small assumption here that if lin_net1 has
   ethernet address "\0lin00" then
   lin_net2 has ethernet address "\0lin01"
*/
static __exit int  lin_net1_rebuild_header(struct sk_buff *skb)
{
	...
	eth->h_dest[ETH_ALEN-1] = '1'; /*lin_net1 -> lin_net2*/
	return 0;
}
...
/* This function has to build the header from the scratch
   ...
   We have to copy "\0lin00" in h_source and
   we have to copy "\0lin01" in h_dest
*/
static __exit int  lin_net1_hard_header(struct sk_buff *skb,
				...
				struct pt_regs *regs)
{
	...
	/* step 1 */
	if( read_from_hardware(lin_net1_d, &data, &len) < 0 )
		return;

	/* step 2 */
	skb = dev_alloc_skb(len+2);
	if( !skb ) {
		printk("lin_net1: Low on memory. Packet dropped\n");
