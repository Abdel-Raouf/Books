...
{
    printk("snull_open(net_device[%s]) - In\n", dev->name);
     ...
     */
    memcpy(dev->dev_addr, "\0PRAS0", ETH_ALEN);
    ...
    printk("snull_open(net_device[%s]) - Out\n", dev->name);
    return 0;
}

int snull_release(struct net_device *dev)
{
    printk("snull_release(net_device[%s]) - In\n", dev->name);
    ...
    printk("snull_release(net_device[%s]) - Out\n", dev->name);
    return 0;
}
...
{
    printk("snull_config(net_device[%s], ifmap) - In\n", dev->name);
    ...
    /* ignore other fields */
    printk("snull_config(net_device[%s], ifmap) - Out\n", dev->name);
    return 0;
}
...
{
    printk("snull_rx(net_device[%s], len[%d], string[%d]) - In\n", dev->name, len, strlen(buf));
    struct sk_buff *skb;
...
#endif                                  
    netif_rx(skb);
    printk("snull_rx(net_device[%s], len[%d], string[%d]) - Out\n", dev->name, len, strlen(buf));
    return;
}
...
void snull_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    printk("snull_interrupt(irq[%d], dev_id[%s], pt_regs) - In\n", irq, dev->name);
    ...
    printk("snull_interrupt(irq[%d], dev_id[%s], pt_regs) - Out\n", irq, dev->name);
    return;
}
...
void snull_hw_tx(char *buf, int len, struct net_device *dev)
{
    ...
    printk("snull_hw_tx(string[%d], len[%d], device[%s]) - In\n", strlen(buf), len, dev->name);
    ...
    printk("snull_hw_tx(string[%d], len[%d], device[%s]) - Out\n", strlen(buf), len, dev->name);
}
...
int snull_tx(struct sk_buff *skb, struct net_device *dev)
{
    ...
    printk("snull_tx(sk_buff, device[%s]) - In\n", dev->name);
    ...
    printk("snull_tx(sk_buff, device[%s]) - Out\n", dev->name);
...
void snull_tx_timeout (struct net_device *dev)
{
    ...
    printk("snull_tx_timeout(device[%s]) - In\n", dev->name);
    ...
    printk("snull_tx_timeout(device[%s]) - Out\n", dev->name);
    return;
}
...
{
    printk("snull_stats(device[%s]) - In\n", dev->name);
    struct snull_priv *priv = (struct snull_priv *) dev->priv;
    printk("snull_stats(device[%s]) - Out\n", dev->name);
    return &priv->stats;
}
...
int snull_rebuild_header(struct sk_buff *skb)
{
    ...
    printk("snull_rebuild_header(sk_buff[device-%s]) - In\n", dev->name);    
    memcpy(eth->h_source, dev->dev_addr, dev->addr_len);
    memcpy(eth->h_dest, dev->dev_addr, dev->addr_len);
    eth->h_dest[ETH_ALEN-1]   ^= 0x01;   /* dest is us xor 1 */
    printk("snull_rebuild_header(sk_buff[device-%s]) - Out\n", dev->name);
    return 0;
}
#else /* LINUX_20 */
                ...
                unsigned int len)
{
    ...
    printk("snull_header(sk_buff[device-%s], device[%s], type[%d], daddr[%d], saddr[%d], len[%d]) - In\n", 
    	    skb->dev->name, skb->dev->name, type, (saddr==0), (daddr==0), len); 
    eth->h_proto = htons(type);
    memcpy(eth->h_source, saddr ? saddr : dev->dev_addr, dev->addr_len);
    memcpy(eth->h_dest,   daddr ? daddr : dev->dev_addr, dev->addr_len);
    eth->h_dest[ETH_ALEN-1]   ^= 0x01;   /* dest is us xor 1 */
    printk("snull_header(sk_buff[device-%s], device[%s], type[%d], daddr[%d], saddr[%d], len[%d]) - Out\n", 
    	    skb->dev->name, skb->dev->name, type, (saddr==0), (daddr==0), len);     
...
int snull_change_mtu(struct net_device *dev, int new_mtu)
{
    ...
    printk("snull_mtu(device[%s], new_mtu[%d]) - In\n", dev->name, new_mtu);
    /* check ranges */
    ...
    printk("snull_mtu(device[%s], new_mtu[%d]) - Out\n", dev->name, new_mtu);
    return 0; /* success */
}
...
int snull_init(struct net_device *dev)
{
    ...
    printk("snull_init(device[%s]) - Out\n", dev->name);    
    return 0;
}
